...
bool Classifier::is_alienated(const cv::Mat &query) {
    ...
    * dimensional space. We want to look at all the points that we have for each
    * label and compute and estimator. Afterwards we check if the query is in
    * the 95% confidence interval or the groups of any label, if not we return
    * true
    */
