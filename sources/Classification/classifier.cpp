...
Classifier::Classifier(int num_people, int dim, std::vector<cv::Mat> &num_reps, std::vector<int> &labels) {
    this->num_people = num_people;
    this->dim = dim;
...
void Classifier::train(std::vector<cv::Mat> &num_reps, std::vector<int> &labels) {
    this->num_reps=num_reps;
    this->labels=labels;
}
